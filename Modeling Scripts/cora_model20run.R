...
base_dir <- "C:/Users/user/Desktop/Thesis Coding/citation_text_min" # contains edges.txt, features.csv, labels.csv
...
use_focal = TRUE
focal_gamma = 2.0
focal_alpha = computed per-seed from train distribution
...
for (seed in seeds) {
  ...
  model_full <- build_e2e_boost_flexible(
    ...
    T = 3L, init_shrink = c(0.85,0.80,0.75),
    k_top = 8L, tau_cos = 0.12, init_alpha_blend = 0.80,
    block_feature_dim = 64L, block_depth = 4L, block_dropout = 0.10,
    lr = 3e-3, l2 = 1e-4,
    use_focal = TRUE, focal_gamma = 2.0, focal_alpha = alpha_vec,
  ...
  model_full <- train_e2e_boost(
    ...
    rounds = 5L, epochs_per_round = 70L,
    hard_thresh = 0.65, up_factor = 3.0,
    verbose = 1L, es_patience = 40L,
    rlrop = TRUE, rlrop_factor = 0.5, rlrop_patience = 8L, rlrop_min_lr = 1e-5
  )
