...
if (!file.exists(hparams_path)) {
 ...
 # boosting
 T = 3L,
 init_shrink = c(0.90, 0.90, 0.85),

 # KNN / similarity
 k_top = 32L, 
 tau_cos = 0.1, # slightly looser than 0.07, tighter than 0.05
...
for (seed in seeds) {
  ...
  pca_dim <- min(192, length(train_idx) - 1L)
  ...
  pca_dim <- min(224L, length(train_idx) - 1L)
  ...
  gf_all <- graph_feats_all(+A_raw); C_all <- gf_all$C; D_all <- gf_all$D
  
  ## -------- 4) Features + heterophily extras --------
  pca_dim <- min(224L, length(train_idx)-1L) # a bit wider for Squirrel
  prep <- fit_feature_prep(X[train_idx, , drop=FALSE], TRUE, TRUE, pca_dim)
  Xp_all <- apply_feature_prep(X, prep)
  
  A_feat <- row_norm(+A_raw); diag(A_feat) <- 1
  Xaug_all <- augment_AX_flex(Xp_all, A_feat, w1=0.10, w2=0.0, post_l2=TRUE)
  
  ## label diffusion
  Y0 <- matrix(0, N, Kc); Y0[train_idx, ] <- to_categorical(y[train_idx], Kc)
  Y1 <- A_feat %*% Y0; Y2 <- (A_feat %*% A_feat) %*% Y0
  Xaug_all <- cbind(Xaug_all, 0.18*Y1, 0.09*Y2)
  
  ## 2-hop-only features
  A_bin <- sign(+A_raw)
  A2 <- sign(A_bin %*% A_bin); diag(A2) <- 0
  A2_only <- A2 - A_bin; A2_only[A2_only < 0] <- 0
  A2_feat <- row_norm(A2_only); diag(A2_feat) <- 1
  X_h2 <- A2_feat %*% Xp_all
  Xaug_all <- cbind(Xaug_all, 0.22*X_h2)
  
  ## GPR-like stack (up to 6 hops)
  H <- Xp_all; Hs <- list(H)
  for (h in 1:5){ H <- A_feat %*% H; Hs[[h+1]] <- H }
  gpr_w <- c(0.32, 0.17, 0.14, 0.12, 0.09, 0.07)
  Xgpr <- do.call(cbind, Map(`*`, Hs, as.list(gpr_w)))
  Xaug_all <- row_l2(cbind(Xaug_all, Xgpr))
