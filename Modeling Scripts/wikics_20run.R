# =========================
# 20-run Cora experiment (E2E-Boost)
# =========================
...
# -------- CONFIG --------
dataset_name <- "WikiCS"
base_dir <- "C:/Users/user/Desktop/Thesis Coding/wikics_text_min"
...
X <- as.matrix(ds$X)
y <- ds$y
if (!is.integer(y)) y <- as.integer(factor(y)) - 1L 
...
for (seed in seeds) {
  ...
  inputs_all <- list(
    X_in = array(Xaug_all, dim = c(1, N, ncol(Xaug_all))),
  ...
  model_full <- build_e2e_boost_flexible(
    n_nodes = N, n_features = ncol(Xaug_all), num_classes = Kc,
    T = 3L,
    init_shrink = c(0.90, 0.85, 0.80),
    k_top = 32L, tau_cos = 0.20, init_alpha_blend = 0.65,
    block_feature_dim = 64L, prev_emb_dim = 16L, gate_dim_target = 96L,
