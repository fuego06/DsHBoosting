...
T = 3L,
 ...
 k_top = 12L, tau_cos = 0.35, init_alpha_blend = 0.6,
 block_feature_dim = 64L, prev_emb_dim = 16L, gate_dim_target = 80L,
 block_depth = 6L, block_dropout = 0.30,
 lr = 2e-3, l2 = 1e-3,
 use_focal = TRUE, focal_gamma = 1.6, focal_alpha = focal_alpha,
 use_cluster_gate = TRUE, gate_beta = 8, gate_normalize_inputs = TRUE
  rounds = 4L,
 epochs_per_round = 140L,
 hard_thresh = 0.55,
 up_factor = 2.0,
 verbose = 1L,
 es_patience = 30L,
...
for (seed in seeds) {
  ...
  pca_dim <- min(100L, length(train_idx) - 1L)
